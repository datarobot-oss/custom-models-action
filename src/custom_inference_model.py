...
class CustomInferenceModel(CustomInferenceModelBase):
    ...
    def _set_filtered_model_paths(self, model_info, included_paths, excluded_paths):
        # NOTE: we would like to keep the relative paths in a model without resolving them.
        included_paths = self._remove_undesired_sub_paths(included_paths)
        if not excluded_paths:
            final_model_paths = included_paths
        else:
            excluded_paths = self._remove_undesired_sub_paths(excluded_paths)
            ...
            for index, included_normpath in enumerate(included_normpaths):
                if included_normpath not in excluded_normpaths:
                    if included_normpath == model_info.model_path.absolute():
                        continue
                    if Path(included_normpath).is_dir():
                        continue
                    final_model_paths.append(included_paths[index])

        model_info.set_model_paths(final_model_paths, self.options.root_dir)
